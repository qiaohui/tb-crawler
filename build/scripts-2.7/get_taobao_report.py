...
gflags.DEFINE_string('start', "", "get report start from yyyymmdd, default today")
...
def dates():
    ...
    interval = 1
    if FLAGS.end != "":
        end = datetime.strptime(FLAGS.end, "%Y%m%d")
    if FLAGS.interval != 0:
        interval = FLAGS.interval
    elif FLAGS.start != "":
        interval =  (end - datetime.strptime(FLAGS.start, "%Y%m%d")).days
    for i in range(interval, 0, -1):
        yield datetime.strftime(end-timedelta(i-1), "%Y%m%d")
    ...
    for d in waitlimit(FLAGS.limit, 60.0, dates()):
        ...
        try:
            ...
            while result_len >= total:
                ...
                pageno += 1
                result_len = len(report['taobaoke_report']['taobaoke_report_members']['taobaoke_report_member'])
                got += result_len
                logger.info("result %s %s -> %s %s", d, pageno, got, len(report['taobaoke_report']['taobaoke_report_members']['taobaoke_report_member']))
                if result_len > 0:
                    members = report['taobaoke_report']['taobaoke_report_members']['taobaoke_report_member']
                    for m in members:
                        try:

...
class TopRequests:
    def __init__(self, req, resp):
        ...
        if not resp or not resp.has_key('items'):
            return
        for r in resp['items']['item']:
...
def get_report(top, date, session, pageno=1, pagesize=40, expire=600):
    ...
    request['fields'] = 'commission_rate,real_pay_fee,app_key,outer_code,trade_id,pay_time,pay_price,num_iid,item_title,item_num,category_id,category_name,shop_title,commission,iid,seller_nick,create_time,confirm_time'
    request['date'] = date
...
def get_taobao_shops(top, nickname, expire=600):
    ...
    request['fields'] = 'sid,cid,nick,title,desc,pic_path,created,modified,shop_score'

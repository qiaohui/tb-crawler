...
def crawl_updated_items():
    if FLAGS.force:
        return crawl_items(resql("""select item.id,item.detail_url,item.num_id from item
                left join tb_crawl_shop_items on tb_crawl_shop_items.iid=item.num_id
                where tb_crawl_shop_items.volume != item.volume
                %s order by item.id desc %s
                """))
    else:
        return crawl_items(resql("""select item.id,item.detail_url,item.num_id from item
                left join tb_crawl_shop_items on tb_crawl_shop_items.iid=item.num_id
                left join shop on item.shop_id=shop.id
                where tb_crawl_shop_items.volume != item.volume and
                item.status=1 and shop.type!=3 and shop.status=1
                %s order by item.id desc %s
                """))
